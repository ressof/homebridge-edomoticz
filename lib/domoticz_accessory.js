...
// HAP references (ESM modules don't share globals)
let Service;
let Characteristic;
let UUID;
...
function eDomoticzAccessory(platform, platformAccessory, IsScene, status, idx, name, uuid, haveDimmer, maxDimLevel, subType, Type, batteryRef, swType, swTypeVal, hwId, hwType, image, eve, hwtimeout, descript) {
    ...
    // Bind HAP references once
    if (!Service || !Characteristic || !UUID) {
        const hap = platform?.api?.hap;
        if (!hap) {
            throw new Error('Missing platform.api.hap (Homebridge HAP reference)');
        }
        Service = hap.Service;
...
eDomoticzAccessory.prototype = {
    ...
    getService: function (name, subtype) {
        ...
        try {
            if (subtype) {
                if (typeof this.platformAccessory.getServiceById === 'function') {
        ...
        if (!service) {
            try {
                var targetService = subtype ? new name(this.name, subtype) : new name(this.name);
    ...
    getColorTempValue: function (service, callback) {
        Domoticz.deviceStatus(this, function (json) {
            ...
            sArray.map(function (s) {
                try {
                    var colorObj = (typeof s.Color === 'string') ? JSON.parse(s.Color) : s.Color;
                    var tValue = Number((colorObj && colorObj.t !== undefined) ? colorObj.t : 0);
                    value = chr.props.minValue + ((chr.props.maxValue - chr.props.minValue) / 255 * tValue);
    ...
    getState: function (callback) {
        const value = Characteristic.CurrentHeatingCoolingState.HEAT;
        // Alternative (same numeric value): const value = 1;
...
//                            callback(dMeterDeviceCharacteristic, val10);
            ...
            case this.Type == "Temp" || this.Type == "Temp + Humidity" || "Temp + Humidity + Baro":
                {
                    if ('undefined' !== typeof message.svalue1) {
            ...
            case this.Type == "Temp" || this.Type == "Temp + Humidity" || this.Type == "Temp + Humidity + Baro":
                {
                    ...
                    if (this.Type == "Temp + Humidity" || this.Type == "Temp + Humidity + Baro") {
                        var service2 = this.getService(Service.TemperatureSensor);
                        if (!service2) {
                            service2 = new Service.TemperatureSensor(this.name);
                        }
                        this.gracefullyAddCharacteristic(service2, Characteristic.CurrentRelativeHumidity).on('get', this.getHumidity.bind(this));
  ...
  //                      this.services.push(infotextService);
    ...
    createTvInputService: function (domoticzInputSourceAccessory) {
        ...
        if (typeof tvService !== undefined) {
            ...
            var sArray = Helper.sortByKey(json.result, "Name");
                sArray.map(function (s) {
                    var inputs = Buffer.from(s.LevelNames, 'base64').toString('utf8').split('|');
